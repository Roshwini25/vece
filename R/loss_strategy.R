...
calculate_productivity_loss_hosp <- function(hospitalisations, los, post_hosp_days, daily_wage) {
  ...
  return(sum(hospitalisations * total_days_lost * daily_wage, na.rm = TRUE))
}
...
calculate_losses <- function(df, params) {
  with(params, {
    ...
    hosp_prod_loss <- calculate_productivity_loss_hosp(df$hospitalisations, los, post_hosp_days, daily_wage)
    ...
    informal_care_loss <- calculate_informal_care_loss(df$pediatric_symptomatic, care_days, daily_wage)
